...
export function Preloader() {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      const soft = Math.min(
        92,
        Math.round((1 - Math.pow(1 - Math.min(1, t), 2)) * 92),
      );
      setPct((p) =>
        Math.max(
          p,
          ready || forced ? Math.min(100, Math.max(soft, p + 3)) : soft,
        ),
      );
  ...
  }, [introDone, ready, forced]);

  /* Failsafe. WebGL can fail on an old driver, in a private mode or behind
	   an extension, and then the first frame never arrives and `ready` stays
	   false forever. Without this the overlay sits at 92 % and hides the whole
	   museum, text and all. After 3.2 s the hall opens regardless of the
	   canvas: a missing sculpture is bad, an unreachable site is worse. */
  useEffect(() => {
    if (introDone || ready) return;
    const id = window.setTimeout(() => setForced(true), 3200);
    return () => window.clearTimeout(id);
  }, [introDone, ready]);
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(
      () => setLine((l) => (l + 1) % LOADER_LINES.length),
      780,
    );
    ...
    >
      ...
      <div>
        ...
        <div className="label" style={{ marginTop: 10, minHeight: "1.6em" }}>
          {forced && !ready
            ? "3D-сцена не ответила — открываю зал без неё"
            : LOADER_LINES[line]}
        </div>
...
export function Terminal() {
  ...
  useHotkey(
    useCallback(
      (e: KeyboardEvent) =>
        e.key === "~" || e.key === "ё" || e.code === "Backquote",
      [],
    ),
    useCallback(() => {
      ...
      >
        {lines.map((l, i) => (
          <div
            key={i}
            style={{
              color: l.startsWith(">") ? "var(--accent)" : "var(--muted)",
            }}
          >
            {l}
          </div>
